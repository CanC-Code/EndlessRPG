...
GLuint GrassRenderer::compileShader(GLenum type, const std::string& source) {
    ...
    if (!success) {
        ...
        LOGE("Shader Compilation Failed (%d): %s", type, infoLog);
...
GLuint GrassRenderer::createProgram(GLuint vShader, GLuint fShader) {
    ...
    if (!success) {
        ...
        LOGE("Render Program Link Error: %s", log);
        return 0;
    }
    return program;
}
...
void GrassRenderer::init() {
    ...
    computeProgram = createComputeProgram(cs);
    ...
    // High-fidelity 7-vertex blade geometry (Triangle Strip)
    float bladeVertices[] = {
        -0.03f, 0.0f, 0.0f,  0.03f, 0.0f, 0.0f,  // Base
        -0.02f, 0.4f, 0.0f,  0.02f, 0.4f, 0.0f,  // Mid
        -0.01f, 0.8f, 0.0f,  0.01f, 0.8f, 0.0f,  // High
         0.0f,  1.1f, 0.0f                       // Tip
    };
