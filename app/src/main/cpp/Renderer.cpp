...
void GrassRenderer::updateInput(float mx, float my, float lx, float ly) {
    ...
    // Adjust camera angles based on swipe deltas
...
// ... [Keep your compileShader, createProgram, and createComputeProgram exactly the same] ...
...
void GrassRenderer::updateAndRender(float time, float dt, int width, int height) {
    ...
    // 3. Apply the virtual thumbstick input to our coordinates
    float speed = 8.0f * dt;
    ...
    // The camera is at camX,Y,Z and looks towards camX+lookDirX, etc.
    buildLookAt(view, camX, camY, camZ, 
