...
float exactElevation(vec2 mapXZ) {
    ...
    float h = noise3(p * 0.01) * 30.0 + noise3(p * 0.03) * 10.0;
    return h;
}
...
void main() {
    ...
    // Simulate Earth Curvature by dropping the vertices based on distance from the camera
    ...
    v_Elevation = y + curvatureDrop; // Normal elevation doesn't include visual drop
    ...
    // Floating Origin Render logic: Center the world locally to avoid jitter
