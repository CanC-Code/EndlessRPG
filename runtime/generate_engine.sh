...
# Purpose: Render pipeline with instanced details, illustrative shader, and fixed orientations.
...
// Procedural hash function for consistent clutter scatter
...
extern "C" {
    JNIEXPORT void JNICALL Java_com_game_procedural_MainActivity_onCreated(JNIEnv*, jobject) {
        ...
        // PENCIL SKETCH SHADER: Applies a screenspace hatching texture based on screenspace UVs.
        // This is where "hand-drawn accuracy" is enforced dynamically in GLSL.
        const char* fS = "#version 300 es\n"
            "precision highp float;\n"
            "in vec3 vc; in vec4 wPos; in vec4 sPos;\n"
            "uniform float isT; uniform vec3 camPos;\n"
            "out vec4 o;\n"
            "void main(){\n"
            "  vec3 col = vc;\n"
            "  if(isT>0.5) { col = vec3(0.18, 0.48, 0.18) * vc; }\n"
            "  // Screenspace Hatching logic\n"
            "  vec2 scrUV = sPos.xy / sPos.w;\n"
            "  // Screenspace sine waves create a stable crosshatch pattern (Pencil look)\n"
            "  float hatch = sin(scrUV.x * 200.0 + scrUV.y * 200.0) * sin(scrUV.x * -200.0 + scrUV.y * 200.0);\n"
            "  float luminance = dot(col, vec3(0.299, 0.587, 0.114));\n"
            "  // Apply sketch pattern in shadowed regions (low luminance)\n"
            "  if(luminance < 0.4 && hatch < 0.0) { col *= 0.6; }\n"
            "  o = vec4(col, 1.0);\n"
            "}";
        ...
        // Screenspace illustrative color logic often uses parchment clear colors
        glClearColor(0.9f, 0.9f, 0.88f, 1.0f);
    ...
    JNIEXPORT void JNICALL Java_com_game_procedural_MainActivity_triggerAction(JNIEnv*, jobject, jint id) {
        if(id==1) slashT=1.0f;     // Restored Attack Action
        if(id==2) block=true;      // Restored Block Action
        if(id==3) block=false;     
        if(id==4 && jumpT<=0) jumpT=1.0f;      // Restored Jump Action
        if(id==6) bashT=1.0f;      // Restored Shield Bash
    }
    ...
    JNIEXPORT void JNICALL Java_com_game_procedural_MainActivity_onDraw(JNIEnv*, jobject, jfloat ix, jfloat iy, jfloat yaw, jfloat pitch, jfloat zoom) {
        ...
        // ORIENTATION FIX: joystick vector is rotated by current camera yaw so forward is true-forward.
        if(fabs(ix)>0.0f || fabs(iy)>0.0f) {
            float s=sin(yaw), c=cos(yaw);
            float dx=ix*c-(-iy)*s, dz=ix*s+(-iy)*c;
        ...
        float jumpY = (jumpT > 0) ? 4.0f * jumpT * (1.0f - jumpT) : 0.0f; // Simplified restored jump curve
        float th = sin(px*0.4f)*cos(pz*0.4f)*1.5f;

        float safeZoom = (zoom < 4.0f) ? 8.0f : zoom; // camera protection
        ...
        // 1. TERRAIN (isT=1.0 for vertex displacement)
        glUniform1f(lt, 1.0f);
        ...
        // 2. PROCEDURAL CLUTTER SCATTER (isT=0.0)
        glUniform1f(lt, 0.0f);
        ...
        // Right Arm (Shield + Bash Animation + Correct orientation)
        float sRot = block ? -1.5f : (bashT > 0 ? -1.8f : sin(wt)*0.5f);
        Mat4 mRArm = tBase.mul(Mat4::trans(-0.38f, 0.65f, 0)).mul(Mat4::rotX(sRot));
        ...
        // Shield attaches to outside of forearm, oriented outward. (Fixed alignment issue)
        ...
        Mat4 mLArm = tBase.mul(Mat4::trans(0.38f, 0.65f, 0)).mul(Mat4::rotX(swRot));
        glUniformMatrix4fv(lm,1,GL_FALSE,mLArm.m); glBindVertexArray(vaoUpLimb); glDrawArrays(GL_TRIANGLES,0,N_UP_LIMB);

        Mat4 mLFore = mLArm.mul(Mat4::trans(0, -0.5f, 0)).mul(Mat4::rotX((slashT > 0) ? -0.5f : 0.0f)); // Twist wrist during strike
        glUniformMatrix4fv(lm,1,GL_FALSE,mLFore.m); glBindVertexArray(vaoLowLimb); glDrawArrays(GL_TRIANGLES,0,N_LOW_LIMB);

        // Sword connects exactly at hand (trans 0, -0.5, 0) and points forward. (Fixed alignment issue)
        Mat4 mSword = mLFore.mul(Mat4::trans(0, -0.5f, 0)).mul(Mat4::rotX(1.57f)); // Point forward
