#!/bin/bash
echo "Injecting Kinematics Physics Engine and Procedural Textures..."
...
extern "C" {
    JNIEXPORT void JNICALL Java_com_game_procedural_MainActivity_onCreated(JNIEnv*, jobject) {
        // PROCEDURAL SHADER: Adds pseudo-random noise for Grass/Dirt variance
        const char* vS = "#version 300 es\nlayout(location=0) in vec3 p; layout(location=1) in vec3 c; uniform mat4 m,v,pr; uniform float isT; out vec3 vc; out vec4 wPos; out vec4 vPos; void main(){ vec4 w=m*vec4(p,1.0); if(isT>0.5) w.y += sin(w.x*0.4)*cos(w.z*0.4)*1.2; wPos=w; vPos=v*w; gl_Position=pr*vPos; vc=c; }";
        
        const char* fS = "#version 300 es\nprecision mediump float; in vec3 vc; in vec4 wPos; in vec4 vPos; uniform float isT; out vec4 o; void main(){ vec3 col = vc; if(isT>0.5){ float noise = fract(sin(dot(wPos.xz, vec2(12.9898,78.233))) * 43758.5453); col = mix(vec3(0.2,0.5,0.1), vec3(0.3,0.4,0.1), noise); } float dist=length(vPos.xyz); float fog=clamp((dist-10.0)/60.0, 0.0, 1.0); vec3 sky=vec3(0.5,0.7,0.9); o=vec4(mix(col,sky,fog), 1.0); }";
        ...
        vaoLimb=createVAO(M_LIMB, N_LIMB); vaoSword=createVAO(M_SWORD, N_SWORD); 
    ...
    JNIEXPORT void JNICALL Java_com_game_procedural_MainActivity_onChanged(JNIEnv*, jobject, jint w, jint h) {
        // CAMERA FIX: Far plane increased to 150.0 to stop ground clipping
    ...
    JNIEXPORT void JNICALL Java_com_game_procedural_MainActivity_onDraw(JNIEnv*, jobject, jfloat ix, jfloat iy, jfloat yaw, jfloat pitch, jfloat zoom) {
        ...
        // SNAPPY COMBAT TIMING
        if(comboState > 0) { 
            st += 0.5f; // Faster swings
            if(st > 3.14f) { comboState = 0; st = 0; } 
        }

        // Sky Color Matches Fog
        ...
        float hipHeight = py + 0.8f + (sin(wt*2.0f)*0.05f * speed); // 0.4 thigh + 0.4 calf = 0.8 anchor
        
        Mat4 v=Mat4::trans(0,0,-zoom).mul(Mat4::rotX(-pitch)).mul(Mat4::rotY(-yaw)).mul(Mat4::trans(-px,-(hipHeight+0.8f),-pz));
        glUniformMatrix4fv(lv,1,0,v.m);
        
        // DRAW TERRAIN (Larger Grid to cover horizon)
        glUniform1f(lt, 1.0f); 
        ...
        glUniformMatrix4fv(lm,1,0,hipL.m); glBindVertexArray(vaoLimb); glDrawArrays(GL_TRIANGLES,0,N_LIMB);
        Mat4 kneeL = hipL.mul(Mat4::trans(0,-0.4f,0)).mul(Mat4::rotX(swingL > 0 ? swingL : 0)); // Bend knee back
        glUniformMatrix4fv(lm,1,0,kneeL.m); glBindVertexArray(vaoLimb); glDrawArrays(GL_TRIANGLES,0,N_LIMB);
        
        // Right Leg
        Mat4 hipR = root.mul(Mat4::trans(-0.15f,0,0)).mul(Mat4::rotX(swingR));
        glUniformMatrix4fv(lm,1,0,hipR.m); glBindVertexArray(vaoLimb); glDrawArrays(GL_TRIANGLES,0,N_LIMB);
        Mat4 kneeR = hipR.mul(Mat4::trans(0,-0.4f,0)).mul(Mat4::rotX(swingR > 0 ? swingR : 0));
        glUniformMatrix4fv(lm,1,0,kneeR.m); glBindVertexArray(vaoLimb); glDrawArrays(GL_TRIANGLES,0,N_LIMB);

        // Arms
        float armL = -swingL * 0.6f; float armR = -swingR * 0.6f;
        
        // Left Arm & Shield (FIXED SHIELD ORIENTATION)
        Mat4 shL = root.mul(Mat4::trans(0.35f,0.5f,0)).mul(Mat4::rotX(block ? -1.0f : armL));
        Mat4 elbL = shL.mul(Mat4::trans(0,-0.4f,0)).mul(Mat4::rotX(block ? -1.5f : -0.2f));
        glUniformMatrix4fv(lm,1,0,shL.m); glBindVertexArray(vaoLimb); glDrawArrays(GL_TRIANGLES,0,N_LIMB);
        glUniformMatrix4fv(lm,1,0,elbL.m); glBindVertexArray(vaoLimb); glDrawArrays(GL_TRIANGLES,0,N_LIMB);
        
        Mat4 shield = elbL.mul(Mat4::trans(0,-0.2f,0.12f)).mul(Mat4::rotX(1.57f)).mul(Mat4::rotY(1.57f)); // Rotated to face outward
        glUniformMatrix4fv(lm,1,0,shield.m); glBindVertexArray(vaoShield); glDrawArrays(GL_TRIANGLES,0,N_SHIELD);
        
        // Right Arm & Sword Combos
        Mat4 shR = root.mul(Mat4::trans(-0.35f,0.5f,0));
        if(comboState == 1) shR = shR.mul(Mat4::rotY(-sin(st)*2.5)).mul(Mat4::rotX(-1.5));
        else if(comboState == 2) shR = shR.mul(Mat4::rotY(sin(st)*2.5)).mul(Mat4::rotX(-1.5));
        else if(comboState == 3) shR = shR.mul(Mat4::rotX(-sin(st)*3.5));
        else shR = shR.mul(Mat4::rotX(armR));
        
        Mat4 elbR = shR.mul(Mat4::trans(0,-0.4f,0)).mul(Mat4::rotX(-0.2f));
        glUniformMatrix4fv(lm,1,0,shR.m); glBindVertexArray(vaoLimb); glDrawArrays(GL_TRIANGLES,0,N_LIMB);
        glUniformMatrix4fv(lm,1,0,elbR.m); glBindVertexArray(vaoLimb); glDrawArrays(GL_TRIANGLES,0,N_LIMB);
        
        Mat4 sword = elbR.mul(Mat4::trans(0,-0.3f,0)).mul(Mat4::rotX(1.57f));
    ...
    JNIEXPORT void JNICALL Java_com_game_procedural_MainActivity_triggerAction(JNIEnv*, jobject, jint id) {
        if(id==1) { 
            ...
            if(comboState == 0 || st > 1.5f) { comboState++; if(comboState > 3) comboState = 1; st = 0; } 
        } 
