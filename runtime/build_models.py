...
# Build modular components using safe assignment
def build_body():
...
def build_cape():
    ...
    bpy.context.object.rotation_euler = (radians(-15),0,0)
...
v_body = export_model("BODY", 0.1, 0.3, 0.8, build_body)
v_head = export_model("HEAD", 0.9, 0.7, 0.6, build_head)
v_cape = export_model("CAPE", 0.8, 0.1, 0.1, build_cape)
v_sword = export_model("SWORD", 0.7, 0.7, 0.75, build_sword)
v_shield = export_model("SHIELD", 0.4, 0.2, 0.1, build_shield)
v_tree = export_model("TREE", 0.2, 0.6, 0.2, build_tree)
